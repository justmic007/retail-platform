...
# -o /app/bin/order: output path
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 \
    go build \
    -ldflags="-w -s" \
    -o /app/bin/order \
    ./services/order/cmd/server/...
...
COPY --from=builder /app/bin/order /order
